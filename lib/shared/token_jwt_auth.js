...
export default function getTokenJwtAuth(provider) {
  ...
  return async function tokenJwtAuth(
    ...
    if (Array.isArray(payload.aud)) {
      if (!payload.aud.every((aud) => acceptedAud.has(aud))) {
        throw new InvalidClientAuth('list of audience (aud) must only include the endpoint url, issuer identifier or token endpoint url');
      }
