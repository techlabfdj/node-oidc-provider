...
export default function getTokenJwtAuth(provider) {
  ...
  return async function tokenJwtAuth(
    ...
    if (
      (typeof header.typ === 'string' && normalizeTyp(header.typ) === 'client-authentication+jwt')
      || ctx.oidc.isFapi('2.0')
    ) {
      if (payload.aud !== ctx.oidc.issuer) {
        throw new InvalidClientAuth('audience (aud) must equal the issuer identifier');
      }
    } else {
